...
public class WebServer {
	 ...
	 * location.
	 */
	static WebIndex w = new WebIndex();

	public static void main(String[] args) throws Exception {
		w = (WebIndex) Index.load("index.db");
		// WebQueryEngine wqe = WebQueryEngine.fromIndex((WebIndex)
		// Index.load("index.db"));
		WebQueryEngine wqe = WebQueryEngine.fromIndex(w);
	...
	/**
	 * This is going to serve the page that shows results of querying the
	 * WebIndex.
	 */
	private void renderSearchResults(BufferedWriter out, HashMap<String, String> params) {
		StringBuilder content = new StringBuilder("<TITLE>Results</TITLE>"
				+ "<body style='background-color: #333333;' link='#0066ff' vlink='#9900ff'> <div style='width:800px; margin:0 auto;'>"
				+ "<a href='/'>" + layoutLogo() + "</a>");

		// Probably add a disclaimer about file links
		Collection<Page> results = engine.query(params.get("query"));

		System.out.println("Number of results for query " + params.get("query") + ": " + results.size());

		if (results.size() > 0) {
			content.append("<p style='color:#FFFFFF'>Here are the results of your query.<br> "
			...
			content.append("<p>Your query returned no results.</p>");
		}
		content.append("<ul>\n");
		for (Page p : results) {
			content.append("<li>" + layoutPageContent(p) + "</li>\n");
		}
		content.append("</ul>\n" + "</div> </body>");

		renderResponse(out, content.toString());
	}
	...
	private void renderHomepage(BufferedWriter out) {
		// Voice Search implemented here.
		String content = "<TITLE>Home</TITLE>"
				+ "<body style='padding-top: 200px; background-color: #333333;'> <div style='width:800px; margin:0 auto;'>"
				+ layoutLogo()
				+ "<form id='form' method='get' action='search' style='width: 50%; margin:0 auto; line-height: 50px;'>"
				+ "<input name='query' class='element text medium' id='q' style='font-size: 16pt; width: calc(100% - 50px); vertical-align: middle' type='text' maxlength='255' value='' "
				+ "placeholder=' Enter search query'/>"
				+ " <img id='speech' onclick='startDictation()' src='//i.imgur.com/cHidSVu.gif' style='width: calc(50px - 1ex); vertical-align: middle'/>"
				+ "</form> <br>"
				+ "<button type='submit' form='form' value='Submit' style='display: block; margin:0 auto; width:100px; height:27px;'>Tsoogle Search</button>"
				+ "</div> </body>" + "<script>" + " function startDictation() {"
				+ "if (window.hasOwnProperty('webkitSpeechRecognition')) {"
				+ " var recognition = new webkitSpeechRecognition();"
				+ "recognition.continuous = false; recognition.interimResults = false; recognition.lang = 'en-US'; recognition.start();"
				+ "recognition.onresult = function(e) {" + "document.getElementById('q').value"
				+ " = e.results[0][0].transcript;" + "recognition.stop();"
	...
	private String layoutLogo() {
		try {
			...
			FileInputStream fis = new FileInputStream(new File("tsoogle2.png"));
	...
	/**
	 * We'll use this to get the HTML that represents our Page object. It might
	 * be better to Encapsulate this within the Page class itself.
	 */
	...
	private void renderResponse(BufferedWriter out, String content) {
		try {
			...
			// An error with this request might not need to bring down the
			// server
